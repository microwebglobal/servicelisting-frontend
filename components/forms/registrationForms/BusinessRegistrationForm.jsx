...
const BusinessRegistrationForm = ({ previousData }) => {
  ...
  const validateStep = (stepNumber) => {
    ...
    switch (stepNumber) {
      case 1:
        if (!formData.business_registration_number?.trim()) newErrors.business_registration_number = 'Required';
        if (!formData.tax_id?.trim()) newErrors.tax_id = 'Required';
        if (!formData.whatsapp_number?.trim()) newErrors.whatsapp_number = 'Required';
        break;
      case 2:
        if (!formData.service_radius?.trim()) newErrors.service_radius = 'Required';
        if (!formData.address?.trim()) newErrors.address = 'Required';
        break;
      case 3:
        formData.employees.forEach((employee, index) => {
          if (!employee.name?.trim()) newErrors[`employees.${index}.name`] = 'Required';
          if (!employee.phone?.trim()) newErrors[`employees.${index}.phone`] = 'Required';
        });
        break;
      case 4:
        if (!formData.documents.business_registration_doc) newErrors.business_registration_doc = 'Required';
        if (!formData.documents.address_proof) newErrors.address_proof = 'Required';
        if (!formData.documents.signed_terms) newErrors.signed_terms = 'Required';
        if (!formData.documents.signed_agreement) newErrors.signed_agreement = 'Required';
        break;
      case 5:
        if (formData.payment_method === 'upi') {
          if (!formData.payment_details.upi.id?.trim()) newErrors['payment_details.upi.id'] = 'Required';
          if (!formData.payment_details.upi.display_name?.trim()) newErrors['payment_details.upi.display_name'] = 'Required';
          if (!formData.payment_details.upi.phone?.trim()) newErrors['payment_details.upi.phone'] = 'Required';
        } else {
          if (!formData.payment_details.bank.name?.trim()) newErrors['payment_details.bank.name'] = 'Required';
          if (!formData.payment_details.bank.branch?.trim()) newErrors['payment_details.bank.branch'] = 'Required';
          if (!formData.payment_details.bank.ifsc?.trim()) newErrors['payment_details.bank.ifsc'] = 'Required';
          if (!formData.payment_details.bank.account_number?.trim()) newErrors['payment_details.bank.account_number'] = 'Required';
        }
        break;
    }
  ...
  const renderBusinessInfo = () => (
    <div className="space-y-4">
      ...
      {renderReadOnlyField('Authorized Person', formData.authorized_person_name)}
      {renderReadOnlyField('Contact Number', formData.authorized_person_contact)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <input
            ...
            className={`p-2 border rounded w-full ${errors.tax_id ? 'border-red-500' : ''}`}
          />
        ...
        <div>
          <input
            ...
            className={`p-2 border rounded w-full ${errors.business_registration_number ? 'border-red-500' : ''}`}
          />
        ...
        <div>
          <input
            ...
            className={`p-2 border rounded w-full ${errors.whatsapp_number ? 'border-red-500' : ''}`}
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <input
            ...
            className={`p-2 border rounded w-full ${errors.service_radius ? 'border-red-500' : ''}`}
          />
          {errors.service_radius && <p className="text-red-500 text-sm mt-1">{errors.service_radius}</p>}
        </div>
      </div>
      <div>
        <textarea
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <input
                ...
                className={`p-2 border rounded w-full ${errors[`employees.${index}.name`] ? 'border-red-500' : ''}`}
              />
              {errors[`employees.${index}.name`] && (
                <p className="text-red-500 text-sm mt-1">{errors[`employees.${index}.name`]}</p>
              )}
            </div>
            <select
            ...
            <input
              ...
              onChange={(e) => handleEmployeeChange(index, 'qualification', e.target.value)}
            ...
            <div>
              ...
              className={`p-2 border rounded w-full ${errors[`employees.${index}.phone`] ? 'border-red-500' : ''}`}
            />
            {errors[`employees.${index}.phone`] && (
              <p className="text-red-500 text-sm mt-1">{errors[`employees.${index}.phone`]}</p>
            )}
          </div>
          <input
            type="tel"
            value={employee.whatsapp_number}
            onChange={(e) => handleEmployeeChange(index, 'whatsapp_number', e.target.value)}
            placeholder="WhatsApp Number"
            className="p-2 border rounded w-full"
          />
        </div>
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          className={`w-full ${errors.business_registration_doc ? 'border-red-500' : ''}`}
        />
        {errors.business_registration_doc && (
          <p className="text-red-500 text-sm mt-1">{errors.business_registration_doc}</p>
        )}
      </div>
      <div>
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          className={`w-full ${errors.signed_agreement ? 'border-red-500' : ''}`}
        />
        {errors.signed_agreement && (
          <p className="text-red-500 text-sm mt-1">{errors.signed_agreement}</p>
        )}
      </div>
      <div>
      ...
      <div className="space-y-2">
        <div>
          ...
          {errors['payment_details.upi.id'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.upi.id']}</p>
          )}
        </div>
        <div>
          <input
          ...
          {errors['payment_details.upi.display_name'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.upi.display_name']}</p>
          )}
        </div>
        <div>
          <input
          ...
          {errors['payment_details.upi.phone'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.upi.phone']}</p>
          )}
        </div>
      </div>
    ) : (
      <div className="space-y-2">
        <div>
          ...
          {errors['payment_details.bank.name'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.bank.name']}</p>
          )}
        </div>
        <div>
          <input
          ...
          {errors['payment_details.bank.branch'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.bank.branch']}</p>
          )}
        </div>
        <div>
          <input
          ...
          {errors['payment_details.bank.ifsc'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.bank.ifsc']}</p>
          )}
        </div>
        <div>
          <input
          ...
          {errors['payment_details.bank.account_number'] && (
            <p className="text-red-500 text-sm mt-1">{errors['payment_details.bank.account_number']}</p>
          )}
        </div>
      </div>
    )}
...
const renderStep = () => {
  switch (step) {
    ...
    case 3: return renderEmployeeDetails();
    case 4: return renderDocuments();
    case 5: return renderPaymentDetails();
    default: return null;
  }
};
