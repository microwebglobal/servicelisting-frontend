            ...
            <TableBody>
              {booking.BookingItems?.map((item) => (
                <TableRow key={item.id}>
                  <TableCell>{item.serviceItem?.name || item.packageItem?.name}</TableCell>
...
const AdminBookings = () => {
  ...
  const fetchServiceProviders = async () => {
    try {
      const providers = await adminBookingService.getServiceProviders();
  ...
  const handleCancelBooking = async (bookingId) => {
    try {
      await adminBookingService.updateBookingStatus(bookingId, 'cancelled', 'Cancelled by admin');
      fetchBookings();
    } catch (error) {
                ...
                <TableRow key={booking.booking_id}>
                    ...
                    >
                      ...
                      <SelectContent>
                        {statusOptions.filter(option => 
                          option.value !== 'all' && option.value !== 'payment_pending'
                        ).map(option => (
                          <SelectItem key={option.value} value={option.value}>
                            {option.label}
                          </SelectItem>
                  ...
                  <TableCell>
                    {booking.status !== 'cancelled' && (
                      <Select
                        value={booking.provider_id?.toString() || "unassigned"}
                        onValueChange={(value) => 
                          value !== "unassigned" && handleAssignProvider(booking.booking_id, value)
                        }
                        disabled={['cancelled', 'completed', 'refunded'].includes(booking.status)}
                      >
                        <SelectTrigger>
