    ...
    <div className="border rounded-lg p-6 bg-white shadow-sm space-y-6">
      ...
      <div className="flex justify-between items-center border-b pb-4">
        ...
        <div className="text-right">
          ...
          <p className={`font-medium capitalize ${getBookingStatusColor(booking.status)}`}>
            ...
            return (
              <div key={index} className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  {item.special_price && (
                    <p className="text-sm text-green-500">Special Price Applied</p>
                  )}
                </div>
              </div>
            );
      ...
      <div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 space-y-3">
          ...
          <div className="flex justify-between">
            ...
            <p className="font-medium">{calculateDuration(booking.start_time, booking.end_time)}</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg space-y-2">
          <div className="flex justify-between">
            ...
            <p className="font-medium">{formatCurrency(payment.subtotal || 0)}</p>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <p className="font-medium">{formatCurrency(payment.tax_amount || 0)}</p>
          </div>
          {payment.tip_amount > 0 && (
            <div className="flex justify-between">
              <p className="text-gray-600">Tip</p>
              <p className="font-medium">{formatCurrency(payment.tip_amount)}</p>
            </div>
          )}
          <hr className="border-gray-200 my-2" />
          <div className="flex justify-between text-lg font-bold">
            <p className="text-gray-800">Total</p>
            <p className="text-indigo-600">{formatCurrency(payment.total_amount || 0)}</p>
          </div>
          <p className="text-sm text-gray-500 mt-2">
            Payment Status: <span className="capitalize">{payment.payment_status}</span>
          </p>
        </div>
      </div>
      ...
      {booking.provider && (
        <div>
          ...
          <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
            <Image
              src={booking.provider.profile_image || "/assets/images/def_pro.webp"}
              width={48}
              height={48}
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{booking.provider.expertise}</p>
