...
export const adminBookingService = {
  ...
  assignServiceProvider: async (bookingId, providerId) => {
    try {
      const response = await api.put(`/admin/bookings/${bookingId}/assign`, { providerId });
      return response.data;
    } catch (error) {
      throw new Error(error.response?.data?.message || "Failed to assign service provider");
    }
  },
  ...
  getServiceProviders: async (cityId) => {
    try {
      ...
      throw new Error(error.response?.data?.message || "Failed to fetch service providers");
    }
  },
